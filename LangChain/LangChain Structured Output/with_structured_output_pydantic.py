#Used Groq instead of HuggingFace because HuggingFace doesn't support Pydantic input
...
class Review(BaseModel):
    ...
    key_themes : list[str] = Field(description = "Identify key themes discussed and write them down in list.")
    summary : str = Field(description = "Generate a brief summary of the review.")
    sentiment : Literal["pos","neg"] = Field(description = "Determine the sentiment of the review.")
    pros : Optional[list[str]] = Field(default = None, description = "List down all the pros of the product in a list.")
    cons : Optional[list[str]] = Field(default = None,description = "List down all the cons of the product in a list.")
    name : Optional[str] = Field(default = None, description = "Mention the name of the reviewer ONLY WHEN IT IS MENTIONED IN THE REVIEW. If not then return None")
